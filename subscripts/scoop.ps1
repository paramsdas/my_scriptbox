...
# set up a startup job to update scoop and apps installed with scoop
$set_startup_job = Get-Content $PSScriptRoot\..\config\config_stripped.yaml | yq '.scoop.setStartUpJob'
if ($set_startup_job -eq "true") {
  Write-Host
  Write-Host "=== Setting up startup job for scoop update ==="
  Write-Host

  Invoke-Expression "powershell $PSScriptRoot\scoop_update.ps1"
}
